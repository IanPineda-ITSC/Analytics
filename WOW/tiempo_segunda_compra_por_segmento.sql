...
DIAS_RECOMPRA AS (
    SELECT DISTINCT
        EMAIL,
        BRAND,
        nth_value(FECHA, 1) OVER (PARTITION BY EMAIL, BRAND ORDER BY FECHA) AS FECHA_PRIMERA_VENTA,
        nth_value(FECHA, 2) OVER (PARTITION BY EMAIL, BRAND ORDER BY FECHA) AS FECHA_SEGUNDA_VENTA,
