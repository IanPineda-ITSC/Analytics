...
DIAS_RECOMPRA AS (
    SELECT DISTINCT
        EMAIL,
        nth_value(FECHA, 1) OVER (PARTITION BY EMAIL ORDER BY FECHA) AS FECHA_PRIMERA_VENTA,
        nth_value(FECHA, 2) OVER (PARTITION BY EMAIL ORDER BY FECHA) AS FECHA_SEGUNDA_VENTA,
...
SELECT
    SEGMENTO,
    avg(DIAS_RECOMPRA) AS TIEMPO_RECOMPRA
FROM
...
ORDER BY
    CASE SEGMENTO
        WHEN 'LOW_VALUE' THEN 1
        WHEN 'MEDIUM_LOW_VALUE' THEN 2
        WHEN 'MEDIUM_HIGH_VALUE' THEN 3
        WHEN 'HIGH_VALUE' THEN 4
        WHEN 'TOP_VALUE' THEN 5
    END
