...
TIEMPO_DE_RECOMPRA_POR_USUARIO AS (
    SELECT
        nth_value(FECHA, 1) OVER (ORDER BY FECHA PARTITION BY EMAIL) AS PRIMERA_COMPRA,
        nth_value(FECHA, 2) OVER (ORDER BY FECHA PARTITION BY EMAIL) AS SEGUNDA_COMPRA
        SEGUNDA_COMPRA - PRIMERA_COMPRA AS TIEMPO_RECOMPRA
    FROM
        TRANSACCIONES_OLO_Y_CLOUD
)
