...
LOYALTY AS (
   SELECT 
    ...
    AND 
        VO.PART_SEARCH NOT IN ('Migracion Saldos') -- DIFERENCIAS DE SALDO POR MIGRACIÒN DE MY STARBUCKS REWARDS A STARBUCKS REWARDS
    AND 
...
SR_TOTAL AS (
    SELECT upper(trim(TIENDA)) AS TIENDA, VENTA, ORDENES, SEM_ALSEA, ANIO_ALSEA FROM LOYALTY
    UNION ALL
    SELECT upper(trim(TIENDA)) AS TIENDA, VENTA, ORDENES, SEM_ALSEA, ANIO_ALSEA FROM SESSIONM
),
